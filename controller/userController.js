...
const loadHome = async (req, res,next) => {
    try {
        ...
        res.render('home',{user : req.session.user , banner: banners})
    }
    catch (error) {
       next(error);
...
const loadOtp = async (req, res,next) => {
    ...
    if (verify) {
        res.render('register', { user: req.body.user, message: "user already exists!" })
    } else {
        ...
        console.log(newOtp);
        res.render("otpPage", { otp: newOtp, mobno: req.body.mno ,user : req.body.name })
    }
}
...
const verifyOtp = async (req, res,next) => {
    ...
    try {
        console.log("here"+req.body.otp+" "+req.body.sendotp);
        if (req.body.sendotp == req.body.otp) {
            const userData = await user.save();
            if (userData) {
                res.render('register', { user: req.body.user, message: "registered successfully" })
            }
            else {
                res.render('register', { user: req.body.user, message: "registration failed!!" })
            }
        } else {

            console.log("otp not match");
            res.render('register', { user: req.body.user, message: "incorrect otp" })
        }
